...
<div class="materi-main-container fs-5" id="halaman15-container">
    ...
    <div class="materi-section shadow-sm rounded-3 py-3 px-3" style="background:#fff;">
        ...
        <div class="materi-section" style="margin-bottom: 0;">
            <div class="kearifan-box" style="background: #fffde7; border-left: 5px solid #ffb300; padding: 16px 18px; border-radius: 10px; font-size: 1.06em; color: #775b08; margin-bottom: 18px; box-shadow: 0 2px 8px #ffb30018;">
                ...
                <audio id="audio-index-4" src="{{ asset('sounds/materi/hal15/4.mp3') }}"></audio>
            </div>
        </div>
        ...
        <div class="materi-image-row justify-content-center mb-2">
            ...
            <div class="materi-image-col" style="max-width:330px">
                <img src="{{ asset('images/materi/ayo-mencoba-3/contoh.png') }}"
    ...
    @for ($no = 1; $no <= $totalSoal; $no++)
        ...
        @endphp
        <div class="materi-section shadow-sm rounded-3 py-3 px-3 soal-step" id="soal-step-{{ $no }}" style="@if($no !== $firstUnanswered)display:none;@endif;background:#fff;">
            <div class="materi-content fw-bold mb-2" style="font-size:1.09rem;">
                {{ $no }}. {{ $soal[$no]['teks'] }}
                <button type="button" onclick="toggleAudio(this)" class="btn-audio" title="Dengarkan" data-id="hal15-{{ $no }}" data-playing="false">🔊</button>
                <audio id="hal15-{{ $no }}" src="{{ asset('sounds/materi/hal15/hal15-' . $no . '.mp3') }}"></audio>
            </div>
            <div class="materi-image-row justify-content-center mb-2">
                <div class="materi-image-col" style="max-width:330px">
                    <img src="{{ asset('images/materi/ayo-mencoba-3/' . $soal[$no]['gambar']) }}" class="img-fluid materi-img rounded-4" alt="Soal {{ $no }}" style="max-width:300px;">
                </div>
            </div>
            @if(!$isAnswered)
                <div class="d-flex justify-content-center mb-2" style="position:relative;">
                    <div style="width: 200px; height: 20px; background: #f1f1f1; border-radius: 8px; position: relative;">
                        <span style="position: absolute; left:0; top:25px; font-size:13px; color:#b91c1c; font-weight:500;">Salah</span>
                        <span style="position: absolute; right:0; top:25px; font-size:13px; color:#388e3c; font-weight:500;">Benar</span>
                        <div id="slider-{{ $no }}" style="position: absolute; left:0; right:0; top:0; bottom:0; z-index:2;"></div>
                    </div>
                </div>
                <div class="d-flex justify-content-center">
                    <div class="slider-instruction-box mb-2 px-3 py-2 rounded" style="background:#f8f9fa;border:1px solid #e0e0e0;display:inline-block;">
            ...
            @else
                <div id="penjelasan-{{ $no }}" class="mt-2">
                    ...
                    <div class="soal-tooltip-jawaban-kunci-row">
                        <span class="badge warna-label yellow-card">
                            Kunci Jawaban: {{ ucfirst($kunciJawab) }}
                        </span>
                    </div>
                    <div class="card card-body mt-2 border-info bg-light d-flex align-items-center gap-3"
                        ...
                        <span>{!! $explain !!}
                            <button type="button" onclick="playPenjelasanAudio({{ $no }}, this, {{ $isCorrect ? 'true' : 'false' }})" class="btn btn-audio ms-2">🔊</button>
                            <audio id="audio-penjelasan-{{ $no }}" src="{{ asset('sounds/materi/hal15/' . ($isCorrect ? 'benar' : 'salah') . $no . '.mp3') }}"></audio>
                        </span>
                    </div>
                </div>
            @endif
            {{-- Penjelasan area tetap tersedia untuk JS jika submit via slider --}}
            @if(!$isAnswered)
                <div id="penjelasan-{{ $no }}" class="mt-2"></div>
            @endif
        </div>
    @endfor
    @endif

    {{-- ...section review, nav footer, script, dll, TIDAK ADA YANG DIUBAH... --}}
        ...
        <div id="review-area">
            ...
            @for ($no = 1; $no <= $totalSoal; $no++)
                @php
                    ...
                    $explain = $isCorrect ? $penjelasan[$no]['benar'] : $penjelasan[$no]['salah'];
                @endphp
                <div class="materi-section shadow-sm rounded-3 py-3 px-3 review-step" id="review-step-{{ $no }}" style="@if($no>1)display:none;@endif;background:#fff;">
                    <div class="materi-content fw-bold mb-2" style="font-size:1.09rem;">
                        {{ $no }}. {{ $soal[$no]['teks'] }}
                        <button type="button" onclick="toggleAudio(this)" class="btn-audio" title="Dengarkan" data-id="hal15-{{ $no }}" data-playing="false">🔊</button>
                        <audio id="hal15-{{ $no }}" src="{{ asset('sounds/materi/hal15/hal15-' . $no . '.mp3') }}"></audio>
    ...
    <div class="materi-nav-footer mt-3">
        <a href="{{ route('admin.materi.halaman14') }}" class="btn btn-nav fs-5">← Sebelumnya</a>
        @if($sudahMenjawab && $skor >= $kkm)
            <a href="{{ route('admin.materi.halaman16') }}" class="btn btn-nav btn-next fs-5">Selanjutnya →</a>
        @else
...
<script>
let sudahMenjawab = @json($sudahMenjawab);
let kunciJawaban = @json($kunci);
let totalSoal = {{ $totalSoal }};
let firstUnanswered = {{ $firstUnanswered }};
let jawabanUser = @json($jawabanUser ?? []);
let penjelasanBlade = @json($penjelasan);
...
function showFeedbackPopup(feedback, idx, benar) {
    ...
    document.getElementById('popup-img').src = benar
        ? "{{ asset('images/materi/ayo-berlatih-3/benar.png') }}"
        : "{{ asset('images/materi/ayo-berlatih-3/salah.png') }}";
    ...
    audio.src = '{{ asset('sounds/materi/hal15/') }}/' + (benar ? 'benar' : 'salah') + idx + '.mp3';
    audio.currentTime = 0;
        ...
        penjelasan.innerHTML = `
            ...
            <div class="card card-body mt-2 border-info bg-light d-flex align-items-center gap-3"
                ...
                <span>${explain}
                    ...
                    <audio id="audio-penjelasan-${idx}" src="{{ asset('sounds/materi/hal15/') }}/${isCorrect ? 'benar' : 'salah'}${idx}.mp3"></audio>
                </span>
            </div>
        `;
    }
    ...
    for (let i = 1; i <= totalSoal; i++) {
        ...
        if (sliderBox && input && !isAnswered) {
            ...
            sliderBox.noUiSlider.on('set', function (values, handle) {
                ...
                input.value = val;
                fetch("{{ route('admin.materi.halaman15.jawab') }}", {
                    method: 'POST',
                ...
                penjelasan.innerHTML = `
                    ...
                    <div class="card card-body mt-2 border-info bg-light d-flex align-items-center gap-3"
                        ...
                        <span>${text}
                            ...
                            <audio id="audio-penjelasan-${i}" src="{{ asset('sounds/materi/hal15/') }}/${isCorrect ? 'benar' : 'salah'}${i}.mp3"></audio>
                        </span>
                    </div>
                `;
            }
