...
<div class="materi-main-container">
    ...
    <div id="stepper-soal" @if(count($jawabanUser) === $totalSoal) style="display:none" @endif>
        ...
        <div class="materi-section mb-5 soal-step" id="soal-step-{{ $no }}" style="@if($no !== $firstUnanswered)display:none;@endif">
            <div class="d-flex align-items-center mb-2">
                <span class="warna-label green-card me-2">Soal {{ $no }}</span>
        ...
        <div id="review-area">
            ...
            <div class="materi-section mb-5 fs-5 review-step" id="review-step-{{ $no }}" style="@if($no>1)display:none;@endif">
                ...
                <div class="drop-area-style text-center mb-3">
                    ...
                    @if($jawab)
                        <img src="{{ asset('images/materi/ayo-mencoba-2/' . $gambarList[$no][$jawab]) }}"
                             ...
                             alt="Jawaban Gambar">
                        <div class="block-option {{ $benar ? 'bg-success' : 'bg-danger' }} text-white d-inline-block px-3 py-2 rounded mt-2" style="cursor: default;" draggable="false">
                            {{ strtoupper($jawab) }}
                        </div>
                    @else
                ...
                <div class="card card-body border-info bg-light mt-2">
                    ...
                    <hr>
                    <span class="text-success"><strong>Kunci Jawaban:</strong> {{ strtoupper($kunci['soal'.$no] ?? '-') }}</span>
                </div>
            </div>
            @endfor
